    ...
    return (
      ...
      <Table>
        ...
        <TableRow className="bg-white dark:border-gray-700 dark:bg-gray-800">
                ...
                <TableCell>{invoice.service.price.currency} {invoice.service.price.amount}</TableCell>
                <TableCell>{invoice.service.price.currency} {invoice.service.price.amount}</TableCell>
              </TableRow>
          { invoice?.invoiceItems != null && invoice.invoiceItems.length &&
            invoice.invoiceItems.map((item, index) => (
              <TableRow className="bg-white dark:border-gray-700 dark:bg-gray-800" key={index}>
                ...
                <TableCell>{item.price.currency} {item.price.amount}</TableCell>
                <TableCell>{item.price.currency} {item.price.amount * item.quantity}</TableCell>
              </TableRow>
            ))}
        </TableBody>
        <TableHead>
        ...
        <TableHead>
          ...
          <TableHeadCell></TableHeadCell>
          <TableHeadCell>Total</TableHeadCell>
          <TableHeadCell>{invoice?.totalPrice.currency} {invoice?.totalPrice.amount}</TableHeadCell>
        </TableHead>
      </Table>
    </div>
    )
